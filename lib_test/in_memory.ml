...
type queues =
  {read: Cstruct.t list ref; write: Cstruct.t list ref; mutable shutdown: bool}
  ...
  with_lock t.m (fun () ->
      ...
      | Listening l ->
          let rec wait () =
            ...
            | x :: xs ->
                l.connecting <- xs ;
                let qs = {read= ref []; write= ref []; shutdown= false} in
                with_lock x.m (fun () ->
                ...
                let y =
                  { x with
                    state=
                      Connected
                        {read= qs.write; write= qs.read; shutdown= false} }
                in
                (y, ())
          in
          let result = wait () in
